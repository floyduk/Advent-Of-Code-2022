...
input_file = open("day19/sample.txt", "r")
...
def robots_i_can_make(robots: list) -> list:
    ...
    # return [b for b in [0, 2 if robots[1] else None, 3 if robots[2] else None, 0] if b != None]
...
max_geodes = 0
def recurse(bp: dict, budget: int, bots: list, mats: list, making: int, bots_made: list):
    global max_geodes

    while budget > 0:
        # Culling test
        if (bots[3] * budget) + (possible_geodes(budget)) <= max_geodes:
            return
        ...
        if making == None:
            for m in robots_i_can_make(bots):
                recurse(bp, budget, [*bots], [*mats], m, [*bots_made, m])
            return 
...
for blueprint in blueprints:
    ...
    for m in [1, 0]:    # At the start we can only afford ore bots or clay bots
        recurse(blueprint, 24, [1,0,0,0], [0,0,0,0], m, [m])
